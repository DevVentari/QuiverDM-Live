...
async function backfillCampaigns(batch: number): Promise<number> {
...
async function backfillSessions(batch: number): Promise<number> {
  let cursor: string | undefined;
  let total = 0;
  while (true) {
...
async function backfillWorldEntities(batch: number): Promise<number> {
  let cursor: string | undefined;
  let total = 0;
  while (true) {
...
async function backfillWorldEntries(batch: number): Promise<number> {
  let cursor: string | undefined;
  let total = 0;
  while (true) {
...
async function main() {
  ...
  if (kind === 'all' || kind === 'campaigns') {
    totals.campaigns = await backfillCampaigns(batch);
  }
  if (kind === 'all' || kind === 'sessions') {
    totals.sessions = await backfillSessions(batch);
  }
  if (kind === 'all' || kind === 'world_entities') {
    totals.world_entities = await backfillWorldEntities(batch);
  }
  if (kind === 'all' || kind === 'world_entries') {
    totals.world_entries = await backfillWorldEntries(batch);
  }
