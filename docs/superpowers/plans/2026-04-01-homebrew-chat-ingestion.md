# Homebrew Chat Ingestion — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the one-shot homebrew media import with a conversational chat interface that keeps the image in context and lets the DM refine extractions through dialogue.

**Architecture:** Two new API routes (`/prepare` for PDF→markdown, `/chat` for stateless Anthropic conversation) plus a full rewrite of `ImportFromMediaDialog`. Client stores full message history and passes it back each turn. Images stay in the first message of history so Sonnet can re-examine them throughout the session.

**Tech Stack:** Next.js 15 App Router, `@anthropic-ai/sdk`, shadcn/ui (Dialog, Input, Select, Button), Lucide icons, Vitest for unit tests.
...
|--------|------|----------------|
| Create | `src/app/api/uploads/homebrew-import/prepare/route.ts` | PDF→markdown via Docling |
| Create | `src/app/api/uploads/homebrew-import/chat/route.ts` | Stateless Anthropic chat endpoint |
| Modify | `src/components/homebrew/import-from-media-dialog.tsx` | Full rewrite — chat UI |
| Create | `src/lib/ai/chat-helpers.test.ts` | Unit tests for pure helper functions |

---

## Task 1: PDF Prepare Endpoint

**Files:**
- Create: `src/app/api/uploads/homebrew-import/prepare/route.ts`

- [ ] **Step 1: Create the route**

```ts
// src/app/api/uploads/homebrew-import/prepare/route.ts
...
```bash
cd E:/Projects/QuiverDM && npx tsc --noEmit --skipLibCheck 2>&1 | grep "prepare/route"
```
...
```bash
git add src/app/api/uploads/homebrew-import/prepare/route.ts
git commit -m "feat(homebrew): add PDF prepare endpoint for chat ingestion"
```

---

## Task 2: Chat API Endpoint — Pure Helpers + Tests

**Files:**
- Create: `src/lib/homebrew-chat-helpers.ts`
- Create: `src/lib/homebrew-chat-helpers.test.ts`

The helpers are pure functions. Extract them first so they can be tested independently before the route is wired up.

- [ ] **Step 1: Create the helpers module**

```ts
// src/lib/homebrew-chat-helpers.ts
...
```ts
// src/lib/homebrew-chat-helpers.test.ts
import { describe, it, expect } from 'vitest';
import { parseItems, stripItemsJson, toAnthropicMessages, selectModel } from './homebrew-chat-helpers';
...
```bash
cd E:/Projects/QuiverDM && npx vitest run src/lib/homebrew-chat-helpers.test.ts
```

Expected: FAIL — `Cannot find module './homebrew-chat-helpers'`
...
```bash
git add src/lib/homebrew-chat-helpers.ts src/lib/homebrew-chat-helpers.test.ts
git commit -m "feat(homebrew): add chat helper functions with tests"
```
...
**Files:**
- Create: `src/app/api/uploads/homebrew-import/chat/route.ts`

- [ ] **Step 1: Create the route**

```ts
// src/app/api/uploads/homebrew-import/chat/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { auth } from '@/lib/auth';
...
```bash
cd E:/Projects/QuiverDM && npx tsc --noEmit --skipLibCheck 2>&1 | grep "chat/route"
```
...
```bash
git add src/app/api/uploads/homebrew-import/chat/route.ts
git commit -m "feat(homebrew): add stateless chat API endpoint"
```

---

## Task 4: Dialog Rewrite

**Files:**
- Modify: `src/components/homebrew/import-from-media-dialog.tsx`

This is a full rewrite. The existing file can be used as a reference for the `select`, `saving`, and `done` steps (copy those sections across), then the `extracting` and `review` steps are replaced with the new `chat` step.
...
```bash
cd E:/Projects/QuiverDM && npx tsc --noEmit --skipLibCheck 2>&1 | grep "import-from-media"
```
...
- [ ] **Step 3: Smoke-test in browser**
  ...
  3. Click Add → "Import from photo / notes"
  4. Verify dialog opens at wider size (`max-w-3xl`)
  5. Verify "Start Chat" button is present
  6. Verify upload, drag-drop, and file list work
  7. Upload one image → click "Start Chat" → verify chat step renders with split panel
  8. Verify items panel shows "Nothing yet" initially, populates as AI responds
  9. Type a message and verify send works
  10. Click paperclip → verify file picker opens
  11. Verify "Save N items" button enables when items exist and triggers save
  12. Verify "Start over" returns to select step

- [ ] **Step 4: Commit**

```bash
git add src/components/homebrew/import-from-media-dialog.tsx
git commit -m "feat(homebrew): conversational chat ingestion dialog"
```
...
**Files:**
- Create: `tests/workflows/homebrew-chat-import.workflow.spec.ts`

- [ ] **Step 1: Write the spec**

```ts
// tests/workflows/homebrew-chat-import.workflow.spec.ts
...
test.describe('homebrew chat import', () => {
  ...
  test('uploads a file, chats to refine, and saves', async ({ page }) => {
    ...
    // Upload a file (use a small PNG from test fixtures if available, else a text file)
    ...
    await expect(page.getByText('Vorpal Edge')).toBeVisible();
  ...
  test('inline item name edit updates the panel', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /start chat/i }).click();
    await expect(page.getByText('Vorpal Edge')).toBeVisible({ timeout: 10_000 });
...
```bash
cd E:/Projects/QuiverDM && npx playwright test tests/workflows/homebrew-chat-import.workflow.spec.ts --project=chromium --reporter=list
```

Expected: 2 tests pass (both use mocked API responses — no Anthropic API key needed).

- [ ] **Step 3: Commit + push**

```bash
git add tests/workflows/homebrew-chat-import.workflow.spec.ts
git commit -m "test(homebrew): workflow spec for chat import dialog"
...
- **Spec coverage:** prepare endpoint ✓, chat endpoint ✓, dialog rewrite ✓, file prep ✓, split panel ✓, items panel editable ✓, add files mid-session ✓, saving/done steps ✓, error handling ✓, workflow spec ✓
- **Types consistent:** `ClientMessage`, `ExtractedItem`, `ReviewItem` defined in Task 2 and imported in Tasks 3 and 4
- **No placeholders:** all code blocks are complete
- **`extract/route.ts` untouched:** confirmed — not referenced in any task
