...
function EncounterDetail({ planId }: { planId: string }) {
  const { campaignSlug, campaignId, currentPlanId, currentSessionId } = useCompendiumRoute();
...
function MonsterDetail({ monsterId }: { monsterId: string }) {
  const { currentPlanId } = useCompendiumRoute();
  const { data: monster } = trpc.homebrew.getContentById.useQuery({ id: monsterId });
...
function ChapterDetail({ chapterId }: { chapterId: string }) {
  const { data: chapter } = trpc.homebrew.getContentById.useQuery({ id: chapterId });
