...
function CampaignsPageInner() {
  ...
  return (
    <PageLayout
      overline="Campaigns"
      title="Your Worlds"
      subtitle="Keep every table, faction, and unfinished thread within reach."
      stats={[
    ...
    >
      <div className="grid gap-6 md:grid-cols-[200px_minmax(0,1fr)] lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <aside className="hidden md:flex flex-col gap-4 lg:sticky lg:top-6 lg:self-start">
          <div className="rounded-[1rem] border border-border/60 bg-card/40 p-4 backdrop-blur space-y-3">
            {[
              { label: 'Campaigns',      value: totalCampaigns },
              { label: 'Sessions Logged', value: totalSessions },
              { label: 'Seats Filled',    value: totalMembers  },
            ].map(({ label, value }) => (
              <div key={label} className="rounded-xl border border-border/50 bg-background/30 px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground/60">{label}</p>
                <p className="font-display text-2xl font-bold text-amber-50 mt-1 tabular-nums">{value}</p>
              </div>
            ))}
          </div>
          <Button onClick={openSheet} className="w-full gap-1.5">
        ...
        <div>
          {campaigns.isLoading ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {(campaigns.data as any[]).map((campaign) => (
                <Link key={campaign.id} href={`/campaigns/${campaign.slug || campaign.id}`}>
                  <div className="stone-card glass-panel h-full cursor-pointer overflow-hidden transition-colors hover:border-amber-700/40">
                    <div className="relative h-28 w-full shrink-0">
                      ...
                      {campaign.status && (
                        <Badge variant="secondary" className="absolute right-2 top-2 text-xs capitalize">{campaign.status}</Badge>
                      )}
                    </div>
                    ...
                    <div className="stone-card-body pt-0">
                      <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1"><Users className="h-3 w-3" />{campaign._count?.members ?? 0}</span>
                      </div>
                    </div>
                  </div>
                </Link>
            ...
            <div className="stone-card">
              <div className="stone-card-body flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="mb-6 max-w-sm text-sm text-muted-foreground">Raise your first world, invite the party, and start building the record of play.</p>
                <Button size="sm" onClick={openSheet}>New Campaign</Button>
              </div>
            </div>
          )}
        </div>
