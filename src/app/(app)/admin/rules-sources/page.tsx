  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 p-6">
      ...
      <p className="text-sm text-muted-foreground">
        Mark PDFs as rules sources to index them for the in-session Rules Companion
        lookup. PDFs must finish processing before indexing.
      </p>

      <div className="space-y-3">
        {pdfs?.map((pdf) => {
          const indexingThisPdf =
            indexMutation.isPending &&
            indexMutation.variables?.pdfId === pdf.id;
          const removingThisPdf =
            removeMutation.isPending &&
            removeMutation.variables?.pdfId === pdf.id;
          ...
          return (
            <Card key={pdf.id}>
              <CardContent className="flex items-center justify-between pt-4">
                ...
                <div className="flex items-center gap-2">
                  {pdf.isRulesSource && (
                    <Badge variant="secondary">Rules Source</Badge>
                  )}
