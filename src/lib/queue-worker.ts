...
import { startPdfToMarkdownConversion, convertPdfWithFallback, type MarkerConversionHandle } from './marker';
...
import { saveExtractedContent } from './gemini-extraction';
...
export function startPDFWorker(concurrency: number = 2) {
  const worker = new Worker<PDFProcessingJobData, PDFProcessingJobResult>(
    ...
    processPDFJob,
    {
      ...
      lockDuration: 60000, // 60 seconds (shorter to detect stalled jobs faster)
